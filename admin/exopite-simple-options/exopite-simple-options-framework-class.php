...
//				$this->config = wp_parse_args( $this->config, $default );
		...
		public function save( $posted_data ) {
			...
			if ( $this->is_metabox() ) {
				...
				if ( $this->is_options_simple() ) {
					$posted_data = absint( $_POST );
				} else {

					if ( isset( $_POST[ $this->unique ] ) ) {
						$posted_data = absint( $_POST[ $this->unique ] );
					} else {
						return false;
					}
		...
		public function get_value( $options, $field ) {
			 ...
			 *
			 *       if ( $unique == $this->segment_for_wordpress && $field['id'] == 'your-field-id' ) {
			 *           // do the magic ;)
			 *       }
			 *
			 *       return $value;
...
/**
 * Use:
 *  $options = ( function_exists( 'get_exopite_sof_option' ) ) ? get_exopite_sof_option( $this->segment_for_wordpress ) : get_option( $this->segment_for_wordpress );
 *  // OR
 *
 *  if ( function_exists( 'get_exopite_sof_option' ) ) {
 *      $options = get_exopite_sof_option( $this->segment_for_wordpress );
 *  } else {
 *      $options = get_option( $this->segment_for_wordpress );
 *  }
 */
if ( ! function_exists( 'get_exopite_sof_option' ) ) {
